...
Fix every error. Treat every warning as a potential user-facing failure. [`--strict` is the right default for a release check](https://code.claude.com/docs/en/plugins-reference) — the base validator's warning-only mode is too permissive for stable distribution.
