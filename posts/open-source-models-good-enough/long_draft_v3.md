...
This post makes the case for a more deliberate approach: match the model to the task, understand where the frontier premium is genuinely justified, and start experimenting with open source models today. Learning takes time. The smart move is to start before you have to.

## What will we cover in this post?

- **How Far Behind Are Open Source Models? What the Benchmarks Actually Show** Honest account of the benchmarks, where frontier models hold a measurable lead, and where the gap narrows to near-irrelevance.
- **Which Tasks Actually Need Frontier Models? A Reasoning-Demand Taxonomy** A 3-tier framework for classifying your workloads by reasoning demand — so you can decide where the frontier premium is justified and where it is waste.
...
- **What Can DeepSeek and Qwen Actually Do? Open Source Models Worth Knowing** DeepSeek and Qwen: what they can do, what they cost, and why the Apache 2.0 licence changes the calculus.
...
- **How Do You Run Your First Open Source Model Experiment? Four Steps.** Four steps. One week. Near-zero risk.

## How Far Behind Are Open Source Models? What the Benchmarks Actually Show
...
## Which Tasks Actually Need Frontier Models? A Reasoning-Demand Taxonomy
...
**Low reasoning demand** — the task is largely pattern-matching, extraction, or rewriting. Examples: summarising a document, classifying an email into categories, extracting structured data from a form, translating text, generating a first draft from a brief, routing a customer support ticket.

Open source models match frontier performance closely on these tasks. The MMLU-Pro numbers above are the right mental model: a gap of 1.2 points rarely translates into a gap a human reviewer can detect.
...
At scale, the cost difference between frontier and open source models ranges from 4:1 to 90:1 depending on the model pair. Open source models on managed inference currently cost between $0.28 and $4.40 per MTok output; leading frontier models cost $15 to $30.
...
## What Can DeepSeek and Qwen Actually Do? Open Source Models Worth Knowing

DeepSeek and Qwen are the two open source model families most relevant to production decisions today. Both achieve competitive scores on knowledge and reasoning benchmarks at a fraction of frontier inference costs.
...
Three reasons to experiment with open source models now, while there is no pressure:
...
## How Do You Run Your First Open Source Model Experiment? Four Steps.
