...
I came to RTK through the same path most people do: a post on X, a number that seemed too good to ignore, and a README that promised 60–90% off the tokens I was spending on shell output. Seventy-three thousand GitHub stars. Apache-licensed. A single Rust binary. The install was four lines.
...
- **Why do big local savings dilute into small bill changes?** — the ceiling arithmetic and the agent-loop effects that can erase local savings entirely.
...
- **RTK can compress by 70%, maintain quality, and barely move total cost — why is that not a contradiction?** — the single takeaway that ties the whole post together.
...
## Why do big local savings dilute into small bill changes?

This is the economic core of the post.

[Weinberger & Hozez (2026)](https://arxiv.org/abs/2607.12161) analysed 2,848 provider-billed Claude Code runs across 103 tasks, seven repositories, and three models. In their corpus, the cost breakdown looked roughly like this:
...
## RTK can compress by 70%, maintain quality, and barely move total cost — why is that not a contradiction?
...
Tools like RTK are attempting something genuinely useful: increasing the information density of what the model sees, not just making context shorter. That is a better target than indiscriminate terseness. But it needs to be measured on your task distribution, not someone else's benchmark.
