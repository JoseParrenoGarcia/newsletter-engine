...
*Why adding more agents is almost always the wrong first move — and a framework for the four cases when it isn't.*

Claude Code agent teams sound like the natural next step once you have subagents working. They are not. They are a coordination layer with a real token cost, known failure modes, and a narrow set of problems they solve well.
...
- **How Claude Code agents communicate.** Three forms of communication, four timing patterns, and what to actually pass between agents.
- **Why agent teams fail.** Eight failure modes with practical mitigations — the section most posts on this topic skip.
- **Choosing between native, DIY, and hybrid agent architectures.** A decision framework for choosing how to orchestrate, without the vendor framing.
...
I built a three-agent review system for my newsletter pipeline — a voice critic, a structure critic, and an impact critic running in parallel. The reason was condition four: independent critique. I needed three agents that had not seen each other's reasoning to score the same draft from different angles. One well-prompted agent reviewing its own output would have had a consistency bias I was trying to avoid. That was a legitimate reason. I kept it because the condition was real. If the only reason had been "more agents feels more thorough," I would have had three agents confidently agreeing with each other for a higher token bill.
...
**Sequential pipeline.** Agents or stages run in a defined order. Later stages consume earlier outputs. This is the easiest pattern to explain to a technical audience because it looks most like ordinary software. Research then synthesis. Drafting then critique. Feature extraction then validation. In a data team's context, this maps directly to ML pipelines: an agent that profiles the raw dataset, followed by one that engineers features from it, followed by one that validates the outputs against a schema and a set of expectation checks. Each stage has a clear contract; failures surface at the boundary.
...
**Parallel specialists, debate, and red team.** Multiple agents examine the same task or related subtasks simultaneously, then vote, critique, or aggregate. Use this when robustness matters more than tidiness: competing hypotheses about a bug, adversarial review of a policy decision, security review from multiple angles. For a data science team, this is the natural shape for model evaluation: three agents assess the same model on accuracy, fairness, and calibration independently, then a synthesis pass surfaces trade-offs across all three. Each assessor has no visibility into the others' conclusions until the aggregation step — that isolation is the point.
...
## How Claude Code agents communicate
...
## Choosing between native, DIY, and hybrid agent architectures
...
If you have decided to go multi-agent and are choosing how to build, here is a concrete starting sequence regardless of which tier you pick:

1. **Name the trigger and write it down.** One sentence: "I am adding a second agent because [context isolation / specialisation / parallel work / independent critique]." If you cannot write that sentence, stop.
2. **Start with one subagent and an explicit output contract before adding a second.** Define what the subagent returns in writing — format, fields, failure signal. Ensure the main session can act on it correctly. Only then add a second agent.
3. **Set a token budget ceiling and an alert threshold before enabling agent teams.** Agent teams consume significantly more tokens. Know your ceiling before you hit it, not after.
4. **Instrument worker-level traces before the first production run.** The top-level session stream is only a condensed view. You cannot debug coordination drift or silent failure without worker-level history. Add tracing before something breaks, not as a response to the first outage.
