...
*What OpenAI and Anthropic's enterprise AI deployment arms actually target, why the consulting layer always returns, and what DS leaders should do about it.*
...
What OpenAI and Anthropic are actually doing is something more specific and more instructive. They are building an AI deployment consulting layer — embedded engineers, partner networks, PE-backed services arms — because they have run into the same wall every major enterprise technology wave runs into. The model works. The organisation does not yet know how to absorb it. Someone has to close that gap. And right now, the labs have decided to be part of who does.
...
- **What Is a Forward-Deployed Engineer in AI Deployment?** The role defined, the Palantir lineage traced, and why the last mile is longer than it looks.
...
- **What DS and Tech Leaders Should Do About AI Deployment Consulting** Three concrete things to do with this information.

---

## What did OpenAI and Anthropic actually launch?

Both labs launched formal AI deployment consulting arms in May 2026. OpenAI created a majority-owned Deployment Company with $4B+ in investment and ~150 Forward Deployed Engineers via the Tomoro acquisition. Anthropic announced a separate PE-backed enterprise AI services company funded by Blackstone, Hellman & Friedman, and Goldman Sachs. Both structures are designed to close the same gap: enterprises accumulating AI pilots that never reach production.
...
The journalism shorthand — "labs becoming consultancies" — compresses something more precise. These are structured deployment arms, not management advisory practices. The work is engineering-led, workflow-specific, and explicitly designed to seed recurring model consumption.

---

## What Is a Forward-Deployed Engineer in AI Deployment?

A forward-deployed engineer (FDE) is an engineer who embeds directly inside an enterprise customer's environment to build, deploy, and iterate on AI systems in context — rather than selling a platform and leaving deployment to the customer. Palantir popularised the model, sending engineers to live inside organisations, map real workflows, and feed what they learned back into the platform. OpenAI and Anthropic are now running the same model for enterprise AI deployment, with FDEs owning complex end-to-end production deployments from day one.

When I first read Anthropic's announcement, the language that jumped out was not the valuation. It was "Applied AI engineers will work alongside the firm's engineering team." That language describes an operating model, not a consulting pitch — and understanding the difference is most of the story.
...
OpenAI is building vertically: it owns its Deployment Company outright, acquired delivery headcount directly through Tomoro, and established co-delivery Frontier Alliances with BCG, McKinsey, Accenture, and Capgemini. Anthropic is building horizontally: its services company is backed by PE firms (Blackstone, Apollo, Goldman Sachs) whose portfolio companies become the natural first customer base, and it is mass-training partner workforces — 30,000 Accenture professionals, the full Deloitte global network — rather than building its own delivery army.
...
The reason these deployments are calibrated only for large enterprise is the same reason a consulting layer has always formed around complex technology: the six recurring jobs between "the software works" and "the organisation extracts value" are expensive to run and impossible to skip.

---

## Why does the consulting layer always return?

The consulting layer returns every time enterprise technology gets powerful enough to matter because the hard work is never the tool — it is the organisation around the tool. Six recurring jobs sit between "the software works" and "the organisation extracts value": translation (capability into workflow), integration (identity, data, systems), change management, governance, value measurement, and political mediation between competing process owners. No software licence covers any of them.

[McKinsey's analysis of ERP transformations](https://www.mckinsey.com/capabilities/tech-and-ai/our-insights/getting-an-erp-transformation-back-on-track) is blunt on why these projects are notoriously over budget: connecting powerful software to real organisations requires workflow redesign, data ownership decisions, and change management for people whose jobs are changing. [Deloitte's intelligent automation survey](https://www.deloitte.com/us/en/insights/topics/talent/intelligent-automation-2022-survey-results.html) on RPA documented the same failure mode — ROI materialised for organisations that moved beyond piloting, but most did not, and the ones that stalled had impressive dashboards full of bot counts and no durable process improvement. RPA coined the term "automation theatre." AI is already rehearsing for its own production run.

AI does not remove any of these six jobs. It intensifies one of them — governance — because AI agents can act, not just report. An ERP system stores process logic. An AI agent may execute parts of it. That distinction changes the accountability model entirely, and it is why regulated industries will demand auditability and human controls that most enterprise pilots have not yet encountered. [UiPath's guidance on scaling automation](https://www.uipath.com/rpa/center-of-excellence) describes the destination: not more bots, but an operating model with defined roles, governance standards, and internal champions who own the discipline. The AI equivalent is an agent registry, a model risk committee, and prompt version governance. Less glamorous. More durable.
...
## What DS and Tech Leaders Should Do About AI Deployment Consulting
...
A concrete starting point: pull three FDE job descriptions from OpenAI, Anthropic, and one cloud provider. Map the skills listed against your current team's profile. The gaps you find are your hiring and development priorities for the next 12 months — not a wishlist, a gap analysis with real market data behind it.
...
I've watched this gap open up in practice. Teams that deployed a model last year are scrambling now because nobody documented who owns the eval suite when the model updates — the deployment worked, but the operating model was never written down. The risk is not direct displacement in the next quarter. It is what happens in the three-to-five-year horizon when the patterns these deployments generate get productised. Every repeatable workflow that an FDE team builds for a large enterprise today is a candidate for a packaged agent, a vertical template, or a platform feature tomorrow. [McKinsey's State of AI](https://www.mckinsey.com/capabilities/quantumblack/our-insights/the-state-of-ai) research consistently shows that the gap between pilot and production is where value is won or lost — and the labs are systematically learning how to close that gap at scale. When those lessons become self-serve products, the downstream effect reaches further down the market.

Pick one pattern your team deploys today — classification, extraction, summarisation, something repetitive and measurable. Ask: in three years, if a lab productises this into a packaged template, does your team's value come from running the template or from knowing which template to use and how to evaluate whether it is working? The answer tells you where to invest right now.
...
A useful starting point: run a half-day session with your team. Three questions on the whiteboard: who owns evals when the model updates, who owns prompt version governance, and what is the escalation path when an agent touches a live workflow. Write the answers down. That one-page decision log is the beginning of an internal AI deployment operating model — and it is the kind of thing that takes an afternoon to produce and months to wish you had done earlier.

---

## What the AI Deployment Wave Means for Your Team
