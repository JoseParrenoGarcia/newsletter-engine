...
*A ground-up guide to subagents — Claude Code's in-product agent primitive — for practitioners who want to understand the feature before building with it.*
...
The [Extend Claude Code documentation](https://code.claude.com/docs/en/features-overview) gives a useful signal for when the upgrade makes sense: "If you're running parallel subagents but hitting context limits, or if your subagents need to communicate with each other, agent teams are the natural next step." Until that transition point, subagents are enough.
...
If you've been building with Claude Code agents:

- What was the task that first made you reach for a subagent instead of a skill? What made the distinction obvious?
- How are you managing tool access in practice — per-agent restriction, or inheriting from the main session?
- Has the description field ever surprised you, either by routing a task you didn't expect, or by failing to activate when you thought it should?
