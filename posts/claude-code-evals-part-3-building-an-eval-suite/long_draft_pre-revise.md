# Claude Code Evals Part 3: The Evaluation Frameworks Landscape
...
That convergence is the point of this post. The eval tooling space looks fragmented from the outside: a dozen frameworks, three clusters of platforms, competing paradigms, different price points. But the underlying architecture is shared. If you understand the five modules, you can work with any tool, pick the right one for your context, and stop being confused by framework marketing.
...
- **What are the five modules every eval framework shares?** — the conceptual DNA that Anthropic named and the entire industry independently rediscovered. A cross-framework mapping table included.
...
- **How does the eval tooling landscape break down?** — three clusters (code-first open-source, tracing-first, enterprise end-to-end), what each cluster gets right, and where they started from.
...
[Claude Code](https://code.claude.com/docs/en/overview) is not a response engine. It reads files, writes code, runs shell commands, calls tools, and spawns subagents that do the same. A single task might involve 30 tool calls across 10 turns. Each of those calls is a decision point. Each decision depends on the ones before it.
...
## What are the five modules every eval framework shares?

The Anthropic engineering post names five modules. I want to lay them out precisely first, then show how every other framework maps onto them — because the mapping is the point.
...
## How does the eval tooling landscape break down?
