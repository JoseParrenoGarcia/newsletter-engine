...
In Part 1, we built an eval for the A/B test document skill. The eval confirmed the document had the right headings, used only the metrics provided in the input, and contained sound statistical reasoning. The skill passed.
...
- **What does the evaluation map look like?** — all nine surfaces where a Claude Code workflow can be evaluated, introduced as a reference table before we go deep on three.
...
## What does the evaluation map look like?
