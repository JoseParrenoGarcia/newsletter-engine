...
| [Claude Code Evals — Part 2: What You Actually Need To Test](#claude-code-evals-part-2-what-you-actually-need-to-test) | Claude Code | The final document is one of eight evaluation surfaces — the dangerous failures are the ones where output looks fine but trigger, trajectory, or file state all failed silently. |
| [Claude Code Evals — Part 3: Anthropic's skill-creator is good enough for your evals](#claude-code-evals-part-3-building-an-eval-suite) | Claude Code | Every eval framework implements the same five modules — Anthropic defined the architecture, the industry rediscovered it, and skill-creator is where Claude Code teams should start. |
| [Sonnet 5 is only good at one thing (beating Sonnet 4.6)](#sonnet-5-one-benchmark-win) | AI Tools & Adoption | Anthropic's launch chart and the community backlash are both wrong about Sonnet 5 — a fair look at what actually changed, what the benchmarks really show, and where the backlash overreached. |
| [Ponytail, Caveman, and the myth of Claude Code token savings](#ponytail-caveman-token-savings-myth) | Claude Code | Two hyped Claude Code skills, three distinct mechanisms, and headline benchmark numbers that shrink hard once independent testing replaces vendor self-reports. |
| [RTK promises to cut your Claude Code token bill. Does it?](#rtk-token-savings) | Claude Code | RTK's 60–90% headline measures eligible Bash output — roughly 3.3% of billed cost — so independent benchmarks find single-digit end-to-end effects, not tens of percent. |
| [Context-mode: what it actually buys your context window](#context-mode-context-window-management) | Claude Code | Context-mode's 98% context reduction is real at the fixture level, but a smaller context window and a smaller token bill are governed by separate mechanics — including an independently confirmed blind spot around external MCP tool calls. |
