# Claude Code: thinking levels, planning mode, and goal mode explained
...
That uncertainty started to bother me when I noticed a quality regression warning from Anthropic's engineering team in April 2026[1]. They had briefly dropped the default effort level from `high` to `medium` during a model update, and performance degraded visibly enough that they reverted it. That single fact told me these settings are not cosmetic. They map directly to how much the model reasons before acting.
...
- **Thinking levels: budgeted deliberation, not intelligence.** What effort levels actually are, how the values map to different models, and when to stop using "high" by default.
- **Planning mode: the organising discipline.** Why agents fail when they act too early, and why plan mode has become my default working posture for anything non-trivial.
- **Planning mode as the umbrella.** How `opusplan` and `/ultraplan` express the same idea in product form — and why model selection makes more sense once you understand planning phases.
- **Goal mode: the completion contract.** How `/goal` works, how it differs from `/loop` and Stop hooks, and why the evaluator mechanism matters.
...
- **Four practical patterns.** Reusable combinations for common real-world situations.

---

## Thinking Levels: Budgeted Deliberation, Not Intelligence

Effort levels sit along what Anthropic calls the test-time-compute curve[1]. The model can reason before it acts — generating internal steps, evaluating approaches, checking assumptions — and effort levels control how much of that reasoning budget it uses.
...
The current values depend on the model you're using[2]:
...
You can change the level mid-session with `/effort <level>` — available since Claude Code v2.1.76[3].

The cost trade-off is real. Higher effort is slower and consumes more tokens. That matters when you're running many sequential tasks or building workflows where latency compounds. Anthropic's April 2026 postmortem noted explicitly that effort levels are the mechanism they expose to set the trade-off between more thinking, lower latency, and fewer usage-limit hits[1].
...
You may have seen the word `ultrathink` in the wild — it circulated in the Claude Code community as a prompt keyword for maximum reasoning[3]. It still works as an in-prompt signal. The official surface for this is `/effort max`, but both route to the same place. The community shorthand is not wrong; it is just not the current official framing.
...
Anthropic's official best practices guide describes a four-phase workflow: Explore, Plan, Implement, Verify[4]. Plan mode enforces the boundary between phases two and three. The agent does not move from planning to implementation until you explicitly allow it.
...
The `Ctrl+G` shortcut opens the proposed plan in your editor before Claude proceeds[4], which means you can annotate, reorder, or discard it before a single file changes.
...
Claude Code's `opusplan` model alias makes this explicit[2]. When you set `/model opusplan`:
...
A second, newer addition is `/ultraplan`, currently in research preview and requiring Claude Code v2.1.91 or later[5]. Ultraplan hands the planning task from the local CLI to a Claude Code web session. Claude drafts the plan in the cloud while your terminal stays free. When the plan is ready, you open it in the browser, comment on specific sections, request revisions, and decide where to execute it — on the web or back in the terminal.
...
You become the scheduler — deciding after every turn whether Claude should continue. Goal mode removes that overhead. The official Claude Code docs describe it as: set a completion condition, and Claude keeps working across turns until a model evaluator confirms the condition holds[6].
...
Goal mode shipped in Claude Code v2.1.139 on May 12, 2026[6][7].
...
The OpenAI Codex cookbook describes the same five components with a clean template[8]:
...
One constraint worth noting: Claude Code's condition field has a 4,000 character limit[6]. That is generous for a well-structured goal — it is only a constraint if you are writing a novel instead of a completion condition.

The underlying principle from the Codex cookbook[8] holds: **a goal is not a longer prompt. It is a completion contract.** The agent needs to know what success looks like in terms it can evaluate, not in terms that only make sense to you.

---

## Does Codex Do the Same?

Yes. OpenAI's Codex has `model_reasoning_effort` (values: minimal, low, medium, high, xhigh), a `/plan` slash command that switches to plan mode, and a `/goal` command for persistent objectives[9][10].

Codex's goal mode became generally available on May 21, 2026 (CLI 0.133.0)[11]. Claude Code's `/goal` shipped nine days earlier, on May 12, 2026 (v2.1.139)[7]. Both tools arrived at the same three-layer control model within the same two-week window.

Codex adds one configuration detail worth noting: `plan_mode_reasoning_effort` is a separate config key[9], which lets you set a different effort level specifically for plan-mode sessions. Claude Code achieves the same outcome through `opusplan`, which routes plan-mode work to a heavier model automatically.
