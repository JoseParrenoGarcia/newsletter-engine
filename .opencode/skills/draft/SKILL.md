---
name: draft
description: "Produces outline.md and long_draft.md from post.yaml, research_brief.md, notes.md, style guides, and reference posts. DO trigger: after brainstorm and research stages are both complete; when a full long-form draft is needed. DO NOT trigger: before brainstorm or research are complete; when the draft already exists and the goal is SEO review or revision (use the seo skill or the revise skill); for short edits or single-section rewrites. Keywords: draft, outline, long_draft, writing, article, style guide, voice, reference posts, tone."
license: proprietary
metadata:
...
Produce `outline.md` and `long_draft.md` for the post in `postFolder` (or the current post folder if none given).
...
- If `stages.brainstorm.status` is not `complete`: stop and say "Run the brainstorm skill first."
...
Invoke a subagent to read all input files in parallel and return their full contents. Pass it the post folder path and the list of files to read:
- `post.yaml`
- `notes.md`
...
**Sequential fallback (no subagent capability):** read each file one at a time in the main session, in the order listed above. This costs more turns but produces the same result.
...
**Standalone mode** (the draft skill run directly):
...
**Pipeline mode** (invoked from the new-post skill with run-all):
...
- **Incremental writing (always):** Never compose the full `long_draft.md` in one response. Write the file section by section — write the frontmatter/H1/subtitle/preview section first, then append one H2 section at a time. Never hold more than one section's worth of prose in a single response — composing the entire draft in one shot risks exceeding the response output cap and losing the write entirely.
